...
class CallbackModule(CallbackBase):
    def __init__(self):
        ...
        env_key = os.environ.get('DATADOG_API_KEY')
        if not env_key:
            print 'Not sending anything to datadog.'
            return

        datadog.initialize(api_key=env_key, api_host=url)

        self._user = os.environ.get('ANSIBLE_DEPLOYER_USER')
        if not self._user:
    ...
    def default_tags(self):
        return ['playbook:{0}'.format(self._playbook_name),
                'started-by:{0}'.format(self._user)]
